...
DNS is one of those technologies you treat as a black box until something breaks at 2am. Then you're staring at a Cloudflare dashboard wondering why an MX record is pointed at a CNAME, and discovering that some validators think that's broken while others don't care.

This is the post that turns DNS from black box to plain table. Each record type, what it does, when to use it, and the gotchas.
